...
function SolarSystemPage() {
  ...
  return (
    <div className={styles.page}>
      ...
      <div id="physics" className={styles.canvasWrapper} ref={canvasWrapperRef}></div>
    </div>
  );
}
  ...
  physicsContext.world.on('collisions:detected', function (data: any) {
    ...
    for (let i = 0; i < data.collisions.length; i++) {
      ...
      if (bodyA.sun) {
        ...
        const newBodyVolume =
          (4 / 3) * Math.PI * Math.pow(bodyA.radius, 3) + (4 / 3) * Math.PI * Math.pow(bodyB.radius, 3);
        ...
        centerOfMassVelocity.x =
          (bodyA.mass * bodyA.state.vel.x + bodyB.mass * bodyB.state.vel.x) / (bodyA.mass + bodyB.mass);
        centerOfMassVelocity.y =
          (bodyA.mass * bodyA.state.vel.y + bodyB.mass * bodyB.state.vel.y) / (bodyA.mass + bodyB.mass);
