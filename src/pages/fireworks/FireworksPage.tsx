...
function FireworksPage() {
  ...
  useEffect(() => {
    window.addEventListener('resize', () => setupCanvas(canvasRef, canvasWrapperRef, setCtx, setScreenWidth, setScreenHeight, animationSettings.current));
    ...
    return () => {
      window.removeEventListener('resize', () => setupCanvas(canvasRef, canvasWrapperRef, setCtx, setScreenWidth, setScreenHeight, animationSettings.current));
...
function setupCanvas(canvasRef: any, canvasWrapperRef: any, setCtx: any, setScreenWidth: any, setScreenHeight: any, animationSettings: any) {
...
function draw(ctx: CanvasRenderingContext2D | null, screenHeight: number, screenWidth: number, animationSettings: any) {
  ...
  if (animationSettings.timer.timerTick >= animationSettings.timer.timerTotal) {
    if (!animationSettings.mouseDown) {
      animationSettings.fireworks.push(new Firework(screenWidth / 2, screenHeight, getRange(screenWidth / 3, 2 * screenWidth / 3), getRange(0, screenHeight / 2), screenWidth));
  ...
  if (animationSettings.timer.limiterTick >= animationSettings.timer.limiterTotal) {
    if (animationSettings.mouseDown) {
      animationSettings.fireworks.push(new Firework(screenWidth / 2, screenHeight, animationSettings.mouseX, animationSettings.mouseY, screenWidth));
