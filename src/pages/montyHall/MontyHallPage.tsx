...
const content = (
  <div>
    <p>
      The Monty Hall Paradox is a classic probability puzzle. You are presented with three doors. Behind one of the
      doors is a prize, and behind the other two are goats. You choose a door, and then the host opens one of the other
      doors to reveal a goat. You are then given the option to stick with your original choice, or switch to the other
      unopened door. What should you do?
    </p>
    <p>
      Common sense might tell you that it's a 50/50 choice whether you stick with your original choice or not. However,
      you mathematically DOUBLE your chances of winning if you switch your choice.
    </p>
    <p>
      Run a test to randomly play 100 games. If the theory holds true, you should win 2x as often when "switching"
      instead of "sticking".
    </p>
  </div>
);
  ...
  useEffect(() => {
    ...
    if (gameSettings.gameState === 'gameOver') {
      ...
      setGameSettings((previousGameSettings) => ({
        ...
        testOutputs: isWin
          ? [...previousGameSettings.testOutputs, 'win']
          : [...previousGameSettings.testOutputs, 'lose'],
      }));
  ...
  return (
    <div className={styles.page}>
      <div className={styles.buttons}>
        ...
        <Button htmlType="button" type="primary" onClick={() => initializeTests(false)}>
          Run 'Stick' Test
        </Button>
        <Button htmlType="button" type="primary" onClick={() => initializeTests(true)}>
          Run 'Switch' Test
        </Button>
      ...
      {gameSettings.gameState === 'initial' && <div className={styles.action}>Choose a door</div>}

      {gameSettings.gameState === 'openLoser' && (
        <div className={styles.action}>
          <Button htmlType="button" type="primary" onClick={openRandomLoser}>
            Open A Losing Door
          </Button>
        </div>
      )}
      ...
      {gameSettings.gameState === 'reveal' && (
        <div className={styles.action}>
          <Button htmlType="button" type="primary" onClick={finishGame}>
            Reveal
          </Button>
        </div>
      )}

      {gameSettings.gameState === 'gameOver' && <div className={styles.action}>{gameSettings.finishText}</div>}

      {gameSettings.gameState === 'testCompleted' && (
        <div className={styles.action}>
          {'Won: ' + gameSettings.numberOfWins + '      Lost: ' + gameSettings.numberOfLosses}
        </div>
      )}
